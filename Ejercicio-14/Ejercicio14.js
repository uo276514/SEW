...
class LectorYMapaDin {
	...
	leerArchivo(files) {
		var archivo = files[0];
		var areaVisualizacion = document.getElementsByTagName('p')[6];
	...
	cogerInfo() {
		if (document.getElementsByTagName('p')[6].textContent == "") {
			alert("Aún no se ha seleccionado el fichero de partidos");
		} else {
			var lines = document.getElementsByTagName('p')[6].textContent.split(";");
	...
	init() {
		...
		this.mapa = new google.maps.Map(document.querySelector('main'),{zoom: 5,center:this.centro});		
	}
}
