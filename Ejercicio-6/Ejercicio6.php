    ...
    class BaseDatos {
        ...
        public function crearBD() {
            ...
            if($this->db->query($query) === TRUE){
                ...
                exit ("<p>ERROR en la creación de la Base de Datos '" . $nombreBD . "'. Error: " . $this->db->error . "</p>");
            }
...
*/
            if($this->db->query($crearTabla) === TRUE){
                ...
                exit ("<p>ERROR en la creación de la tabla '" . $nombreTabla . "'. Error: " . $this->db->error . "</p>");
            }
        ...
        public function insertarDatos() {
            ...
            if ($dni == "") {
                $this->db->close();
                exit("<p>ERROR: dni no puede ser vacío</p>");
            } else if ($nombre == "") {
                $this->db->close();
                exit("<p>ERROR: nombre no puede ser vacío</p>");
            } else if ($apellidos == "") {
                $this->db->close();
                exit("<p>ERROR: apellidos no puede ser vacío</p>");
            } else if ($email == "") {
                $this->db->close();
                exit("<p>ERROR: email no puede ser vacío</p>");
            } else if ($telefono == "") {
                $this->db->close();
                exit("<p>ERROR: telefóno no puede ser vacío</p>");
            } else if ($edad <= 0) {
                $this->db->close();
                exit("<p>ERROR: edad no puede ser inferior o igual a 0</p>");
            } else if ($sexo != "M" && $sexo != "F") {
                $this->db->close();
                exit("<p>ERROR: sexo debe ser M o F</p>");
            } else if ($pericia != "" && ($pericia < 0 || $pericia > 10)) {
                $this->db->close();
                exit("<p>ERROR: pericia debe ser entre 0 y 10</p>");
            } else if ($tiempo != "" && $tiempo < 0) {
                $this->db->close();
                exit("<p>ERROR: tiempo debe ser al menos 0</p>");
            } else if ($valoracion != "" && ($valoracion < 0 || $valoracion > 10)) {
                $this->db->close();
                exit("<p>ERROR: valoración debe ser entre 0 y 10</p>");
            }
        ...
        public function buscarDatos() {
            ...
            if ($dni == "") { 
                $this->db->close();
                exit("<p>ERROR: dni a buscar no puede ser vacío</p>"); 
            }
            ...
            if ($resultado->fetch_assoc() != NULL) {
                ...
                $this->db->close();
                exit("No se ha encontrado ningún dni que coincida");
            }
    ...
    echo "  
            ...
            <h2>Generación de informe de datos de la tabla:</h2>
            ...
            <h2>Importación de datos de CSV a tabla:</h2>
            ...
            <h2>Exportación de datos de la tabla a CSV:</h2>
