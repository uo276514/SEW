	...
	cogerInfoKML() {
		if (document.getElementById("areaVisualizacion").textContent == "") {
			...
			for(var i=0; i < lines.length; i++) {
				if (lines[i].length >= 16 && (lines[i].match("<name>Nacimiento") || lines[i].match("<name>Fallecimiento"))) {
...
class MapaDin {
	...
	init() {
		...
		this.mapa = new google.maps.Map(document.getElementById('mapa'),{zoom: 6,center:this.madrid});		
	}
}
